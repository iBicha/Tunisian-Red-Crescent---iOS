...
public class HTYTextField: UITextField {
    ...
    public func StartAnimation(duration : Double){
        UIView.animateWithDuration(duration, delay: 0.0, usingSpringWithDamping: 0.4, initialSpringVelocity: 0, options: .CurveEaseIn, animations: { () -> Void in
    ...
    public func EndAnimation(duration : Double){
        UIView.animateWithDuration(duration, delay: 0.0, usingSpringWithDamping: 0.4, initialSpringVelocity: 0, options: .CurveEaseIn, animations: { () -> Void in
            if self.text!.isEmpty {
                self.fakePlaceholderLabel.transform = CGAffineTransformTranslate(self.fakePlaceholderLabel.transform, -self.translateX, 0.0)
    ...
    public func SetText(text: String, delay: Double = 0.5){
        if !(self.text!.isEmpty && text.isEmpty) {
            ez.runThisAfterDelay(seconds: delay, after: {
